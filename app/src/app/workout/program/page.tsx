...
export default function ProgramGeneratorPage() {
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: surfaceBg(t), pb: 12 }}>
      {/* Header */}
      <Box sx={{
        px: 2,
        py: 1.5,
        ...
        <Stack direction="row" alignItems="center" spacing={2}>
          ...
          <Typography variant="h6" sx={{ fontWeight: 600, color: tc.h(t) }}>Générer un programme</Typography>
        </Stack>
      </Box>
      ...
      <Box sx={{ px: 2, py: 3 }}>
        <Stepper
          ...
          sx={{
            ...
            '& .MuiStepLabel-label': { color: tc.f(t), fontSize: '0.75rem' },
            '& .MuiStepLabel-label.Mui-active': { color: GOLD },
            '& .MuiStepLabel-label.Mui-completed': { color: tc.m(t) },
...
function GoalStep({ selected, onSelect }: { selected: ProgramGoal; onSelect: (goal: ProgramGoal) => void }) {
  ...
  return (
    <Stack spacing={1}>
      <Typography variant="subtitle1" textAlign="center" sx={{ fontWeight: 600, mb: 1, color: tc.h(t) }}>
        Objectif
      </Typography>
      {goals.map((goal) => (
        <Box
        ...
        >
          <Box sx={{ py: 1, px: 2 }}>
            <Stack direction="row" alignItems="center" spacing={1.5}>
              ...
              <Box sx={{ flex: 1 }}>
                <Typography variant="body1" sx={{ fontWeight: 500, color: tc.h(t) }}>
                  {GOAL_LABELS[goal]}
                </Typography>
...
function FrequencyStep({
  ...
  return (
    <Stack spacing={2}>
      <Typography variant="subtitle1" textAlign="center" sx={{ fontWeight: 600, color: tc.h(t) }}>
        Fréquence
      </Typography>
      ...
      <Box sx={card(t, { p: 3 })}>
        ...
        <Typography variant="body2" textAlign="center" sx={{ mb: 2, color: tc.m(t) }}>
          jours / semaine
        </Typography>
        <Slider
          ...
          sx={{
            px: 2,
            color: GOLD,
            '& .MuiSlider-markLabel': { color: tc.f(t) },
...
function PreviewStep({ program }: { program: GeneratedProgram }) {
  ...
  return (
    <Stack spacing={2}>
      <Typography variant="h6" textAlign="center" sx={{ mb: 1, color: tc.h(t) }}>
      ...
      <Stack direction="row" spacing={1} justifyContent="center" flexWrap="wrap" sx={{ gap: 0.5 }}>
        ...
        <Chip label={APPROACH_LABELS[program.config.approach]} variant="outlined" size="small" sx={{ color: tc.m(t), borderColor: d ? alpha('#ffffff', 0.15) : alpha('#000000', 0.12) }} />
        <Chip label={SPLIT_LABELS[program.config.split]} variant="outlined" size="small" sx={{ color: tc.m(t), borderColor: d ? alpha('#ffffff', 0.15) : alpha('#000000', 0.12) }} />
        <Chip label={`${program.config.daysPerWeek}j/sem`} variant="outlined" size="small" sx={{ color: tc.m(t), borderColor: d ? alpha('#ffffff', 0.15) : alpha('#000000', 0.12) }} />
      </Stack>

      {program.workouts.map((workout, idx) => (
        <Box key={idx} sx={card(t, { p: 2 })}>
              ...
              <Box key={exIdx}>
                <Stack direction="row" alignItems="center" justifyContent="space-between">
                  <Stack direction="row" alignItems="center" spacing={1.5}>
                    ...
                    <Box>
                      <Typography variant="body2" sx={{ fontWeight: 500, color: tc.h(t) }}>
                        {ex.exerciseName}
                      </Typography>
                      <Typography variant="caption" sx={{ color: tc.m(t) }}>
                ...
                {ex.notes.length > 0 && (
                  <Typography variant="caption" sx={{ display: 'block', ml: 5, mt: 0.5, fontStyle: 'italic', color: tc.f(t) }}>
                    {ex.notes[0]}
                  </Typography>
                )}
