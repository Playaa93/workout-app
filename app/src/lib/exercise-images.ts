...
/**
 * Get image URLs for an exercise (typically 2 images: start and end position).
 */
export function getExerciseImages(exerciseNameEn: string | null): string[] {
  if (!exerciseNameEn) return [];
  const id = IMAGE_MAP[exerciseNameEn];
  if (!id) return [];
  return [`${BASE_URL}/${id}/0.jpg`, `${BASE_URL}/${id}/1.jpg`];
}

/**
 * Get the primary (start position) image URL for an exercise.
 */
...
/**
 * Check if images are available for an exercise.
 */
export function hasExerciseImages(exerciseNameEn: string | null): boolean {
  return !!exerciseNameEn && exerciseNameEn in IMAGE_MAP;
}
