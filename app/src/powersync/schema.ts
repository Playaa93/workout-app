...
// =====================================================
// PowerSync Schema - mapped from PostgreSQL schema
// Column types: text, integer, real only
// Timestamps -> text (ISO 8601)
// decimal -> text (preserve precision)
// jsonb / text[] -> text (JSON serialized)
// pgEnum -> text
// =====================================================

const users = new Table({
  email: column.text,
  // password_hash EXCLUDED (security)
  display_name: column.text,
  avatar_url: column.text,
  created_at: column.text,
  updated_at: column.text,
  last_login_at: column.text,
  is_active: column.integer, // boolean -> integer (0/1)
});
...
const morpho_profiles = new Table({
  ...
  morphotype_score: column.text, // jsonb -> text
  wrist_circumference: column.text, // decimal -> text
  ...
  strengths: column.text, // text[] -> JSON text
  weaknesses: column.text,
  recommended_exercises: column.text,
  exercises_to_avoid: column.text,
  questionnaire_responses: column.text, // jsonb -> text
  created_at: column.text,
  updated_at: column.text,
});
...
const morpho_questions = new Table({
  ...
  options: column.text, // jsonb -> text
...
const workout_templates = new Table({
  ...
  name: column.text,
  description: column.text,
  target_muscles: column.text, // text[] -> JSON
...
const user_gamification = new Table({
  ...
  avatar_customization: column.text, // jsonb -> text
  created_at: column.text,
  updated_at: column.text,
});
...
const achievements = new Table({
  ...
  category: column.text,
  requirement: column.text, // jsonb -> text
...
const activity_log = new Table({
  ...
  reference_id: column.text,
  metadata: column.text, // jsonb -> text
  created_at: column.text,
});
