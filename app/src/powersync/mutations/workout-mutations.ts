...
export function useWorkoutMutations() {
  ...
  async function addSet(
    ...
    if (weight > 0 && reps > 0 && !isWarmup) {
      const currentPR = await db.getOptional<{ value: string }>(
    ...
    if (isPr) {
      const existingPR = await db.getOptional<{ id: string }>(
        `SELECT id FROM personal_records WHERE user_id = ? AND exercise_id = ? AND record_type = '1rm'`,
        [userId, exerciseId]
      );
      ...
      if (existingPR) {
        await db.execute(
          ...
          [estimated1RM.toFixed(2), id, now, existingPR.id]
        );
      } else {
        await db.execute(
  ...
  async function endWorkoutSession(
    ...
    const session = await db.getOptional<{ started_at: string }>(
    ...
    // Calculate volume and PR count from local sets
    const stats = await db.getOptional<{
      total_volume: number;
      pr_count: number;
    }>(
      `SELECT
    ...
    const weightRow = await db.getOptional<{ weight: string }>(
  ...
  async function endCardioSession(
    ...
    const session = await db.getOptional<{ started_at: string; cardio_activity: string }>(
    ...
    // Estimate calories (~8 MET for moderate cardio)
    const weightRow = await db.getOptional<{ weight: string }>(
      `SELECT weight FROM measurements WHERE user_id = ? AND weight IS NOT NULL ORDER BY measured_at DESC LIMIT 1`,
      [userId]
    );
